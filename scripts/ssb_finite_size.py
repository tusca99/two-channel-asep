...
def scan_L(alpha, beta, Ls, n_steps, warmup, seed=0):
    """Return |drho| (mean |rho1-rho2|) vs L for a fixed (alpha, beta)."""
    asyms = []
    for L in Ls:
        samples = collect_joint_samples(alpha, beta, L, n_steps, warmup, seed)
        _, asym = bimodality(samples)
        asyms.append(asym)
...
def main():
    ...
    for a, b, label in cases:
        asyms = scan_L(a, b, Ls, n_steps, warmup)
        ax.plot(Ls, asyms, "o-", label=rf"{label}, $\alpha={a}$, $\beta={b}$")
        print(f"{label}: |drho| = {np.round(asyms, 3)}")

    ax.set_xlabel(r"$L$")
    ax.set_ylabel(r"$\langle |\rho_1 - \rho_2| \rangle$")
    ax.set_title("Finite-size scaling of SSB asymmetry")
